...
describe("YAML serialization helpers", () => {
  describe("serializeYamlValue", () => {
    ...
    it("quotes strings with colons followed by space", () => {
      expect(serializeYamlValue("My Title: Subtitle")).toBe(
        '"My Title: Subtitle"'
      );
    });
  ...
  describe("formatPublicationDate", () => {
    it("returns YYYY-MM-DD for standard ISO timestamp", () => {
      ...
      expect(formatPublicationDate("2023-10-27T00:00:00-05:00")).toBe(
        "2023-10-27"
      );
    });
...
describe("formatClip", () => {
  ...
  it("returns raw markdown when frontmatter is disabled", () => {
    expect(formatClip(pageClip, presetWithoutFrontmatter)).toBe(
      pageClip.markdown
    );
  ...
  it("omits frontmatter when preset allows it but frontmatter is disabled via options", () => {
    const formatted = formatClip(pageClip, presetWithFrontmatter, {
      frontmatterEnabled: false,
    });
    expect(formatted).toBe(pageClip.markdown);
  });
});
